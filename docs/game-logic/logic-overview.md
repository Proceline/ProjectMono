...
- `Assets/Scripts/MonopolyPrototype/Prototype3DBoardView.cs`
  - Presentation-only runtime view that maps `PrototypeMapData` to the board platform and an ordered set of `PrototypeBoardTileView` prefab instances.
  - Keeps prefab visuals below the logical `BoardTile` roots so a future tile prefab or art replacement can change presentation without changing movement rules.
- `Assets/Scripts/MonopolyPrototype/PrototypeBoardTileView.cs`
  - Presentation-only component placed on `Assets/Prefabs/PrototypeBoardTile.prefab`.
...
The runtime layout uses the map's square dimensions together with `boardCenter`, `tileSpacing`, and `tileScale`. `Prototype3DBoardView` maps the existing layout's grid X/Y coordinates to world X/Z coordinates, instantiates `Assets/Prefabs/PrototypeBoardTile.prefab` once per ordered route tile, and keeps the board platform separate from the tile prefab. When `fitCameraToBoard` is enabled, `PrototypeBootstrapper` positions a desktop-friendly perspective camera near the board normal with a small tilt so the flat board and 3D mesh thickness remain readable.
...
- `PrototypeMapPainterWindow` is intentionally data-only; runtime visuals remain composed by `PrototypeBootstrapper` and `Prototype3DBoardView` from the replaceable `PrototypeBoardTile` prefab.
